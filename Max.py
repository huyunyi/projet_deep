...
#Y1 = tf.nn.leaky_relu(x1,alpha=0.1)
#Y1 = tf.nn.relu(x1)
Y1 = tf.maximum(x1,tf.nn.sigmoid(x1))
x2 = tf.nn.conv2d(Y1, W2, strides=[1,1,1,1], padding='SAME') + B2
#Y2 = x2*tf.nn.sigmoid(beta2*x2)
#Y2 = tf.nn.leaky_relu(x2,alpha=0.1)
#Y2 = tf.nn.relu(x2)
Y2 = tf.maximum(x2,tf.nn.sigmoid(x2))
...
#Y3 = tf.nn.leaky_relu(x3,alpha=0.1)
#Y3 = tf.nn.relu(x3)
Y3 = tf.maximum(x3,tf.nn.sigmoid(x3))
